...
#Region "Initialization"
    Public Sub ShowForm()
        If LoadFromXML("Inecom_SDK_Reporting_Package.ncmARAging6B.srf") Then

            oForm = SBO_Application.Forms.Item("ncmARAging6B")
            oForm.Title = "AR Ageing 6 Buckets Report"
    ...
    Private Sub PopulateData()
        Try
            ...
            sQuery = " SELECT * FROM " & oCompany.CompanyDB & ".""@NCM_BUCKET"" WHERE ""U_Type"" = 'NCM_AR_AGINGP_6B'"
...
#Region "General Functions"
    Private Function IsSharedFileExist() As Boolean
        Try
            ...
            Select Case RptType
                Case 0
                    g_sReportFilename = GetSharedFilePath(ReportName.ARAging6B_Details)
                    ...
                    End If
                Case 1
                    g_sReportFilename = GetSharedFilePath(ReportName.ARAging6B_Summary)
                    If g_sReportFilename <> "" Then
                        If IsSharedFilePathExists(g_sReportFilename) Then
                            Return True
                        End If
        ...
        Try
            ...
            Try
                If (ExecuteProcedure()) Then
                    ...
                    'Version 03.06.2005 - user defined aging range in days
        ...
        Try
            ...
            For iCount = 1 To 5
                ...
                If (i1 < -1) Then
                    ...
                    SBO_Application.StatusBar.SetText("[ArAging][ValidateParameters] - Value in bucket " & iCount.ToString() & " is less than zero", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    Return False
                End If

                If (i2 < -1) Then
                    oForm.ActiveItem = sTemp2
                    SBO_Application.StatusBar.SetText("[ArAging][ValidateParameters] - Value in bucket " & (iCount + 1).ToString() & " is less than zero", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    Return False
                End If

                If (i1 > i2) Then
                    oForm.ActiveItem = sTemp1
                    SBO_Application.StatusBar.SetText("[ArAging][ValidateParameters] - Value in bucket " & (iCount).ToString() & " is greater than value in bucket " & (iCount + 1).ToString(), SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    Return False
                End If
                ...
                If (iCount = 5) Then
                    If (i1 <> i2) Then
                        oForm.ActiveItem = sTemp1
                        SBO_Application.StatusBar.SetText("[ArAging][ValidateParameters] - Value in bucket " & (iCount).ToString() & " is not equal to value in bucket " & (iCount + 1).ToString(), SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                        Return False
                    End If
                End If
            Next
                        ...
                        Select Case pVal.ItemUID
                            Case "txtB5Val"
                                If (pVal.ItemChanged) Then
                                    oForm.DataSources.UserDataSources.Item("txtB6Val").ValueEx = oForm.DataSources.UserDataSources.Item("txtB5Val").ValueEx
                                End If
                        End Select
                    End If
                End If
