...
Public Class frmAPPayment
    ...
    Private dsAPPayment As System.Data.DataSet
    ...
    Const C_FILE_SRF As String = "Inecom_SDK_Reporting_Package.NCM_AP_PAYMENT.srf"
    Const C_FRM_AP_PAYMENT As String = "NCM_AP_PAYMENT"
...
#Region "General Functions"
    Public Sub LoadForm()
        Try
            ...
            For i As Integer = 0 To SBO_Application.Forms.Count - 1
                If SBO_Application.Forms.Item(i).TypeEx = C_FRM_AP_PAYMENT Then
                    SBO_Application.Forms.Item(i).Close()
                    Exit For
                End If
            Next

            If LoadFromXML(C_FILE_SRF) = True Then
                oForm = SBO_Application.Forms.Item(C_FRM_AP_PAYMENT)
    ...
    Private Sub DefineUserDataSource()
        Try
            oForm.DataSources.UserDataSources.Add("txtBPFr", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 20)
            oEdit = oForm.Items.Item("txtBPFr").Specific
            oEdit.DataBind.SetBound(True, , "txtBPFr")
            oEdit.ChooseFromListUID = "CFL_BP_Fr"
            oEdit.ChooseFromListAlias = "CardCode"

            oForm.DataSources.UserDataSources.Add("txtBPTo", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 20)
            oEdit = oForm.Items.Item("txtBPTo").Specific
            oEdit.DataBind.SetBound(True, , "txtBPTo")
            oEdit.ChooseFromListUID = "CFL_BP_To"
            oEdit.ChooseFromListAlias = "CardCode"

            oForm.DataSources.UserDataSources.Add("txtDateFr", SAPbouiCOM.BoDataType.dt_DATE)
            oEdit = oForm.Items.Item("txtDateFr").Specific
            oEdit.DataBind.SetBound(True, , "txtDateFr")

            oForm.DataSources.UserDataSources.Add("txtDateTo", SAPbouiCOM.BoDataType.dt_DATE)
    ...
    Private Function PrepareDataset() As Boolean
        ...
        Try
            dsAPPayment = New dsPayment
            dt = dsAPPayment.Tables("dtPayment")
            dt.Clear()
            ...
            sQuery = "Exec NCM_RPT_PAYMENT '" & sDateFrom & "','" & sDateTo & "','" & sBPFrom & "','" & sBPTo & "','AP'"
    ...
    Private Sub LoadViewer()
        Try
            ...
            If PrepareDataset() Then
                ...
                frm.ReportName = ReportName.APPayment
                frm.ReportPath = g_sReportFileName
                frm.ReportDataSet = dsAPPayment
    ...
    Private Function IsSharedFileExist() As Boolean
        Try
            g_sReportFileName = GetSharedFilePath(ReportName.APPayment)
