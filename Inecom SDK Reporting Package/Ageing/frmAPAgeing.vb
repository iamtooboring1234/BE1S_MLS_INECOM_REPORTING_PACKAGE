...
#Region "Global Variables"
    ...
    Private g_bCustom_APAgeingCurrency As Boolean = False
    Private g_sCustom_APAgeingCurrencyRptFile As String = ""
...
#Region "Initialization"
    Public Sub ShowForm()
        If LoadFromXML("Inecom_SDK_Reporting_Package.ncmAPAgeing.srf") Then
            ...
            oForm.DefButton = "btnExecute"

            GetCustomizedReportSetting()
            AddDataSource()
            SetupChooseFromList()
            g_sAll = "N"
            oForm.Visible = True
    ...
    Private Sub AddDataSource()
        Try
            ...
            If g_bCustom_APAgeingCurrency Then
                oCombo.ValidValues.Add("2", "Summary By Currency")
            End If
...
#Region "General Functions"
    Private Function IsSharedFileExist() As Boolean
        Try
            ...
            Select Case RptType
                ...
                Case 2
                    sQuery = " SELECT IFNULL(""STRUCTUREPATH"",'') FROM ""@NCM_RPT_STRUCTURE"" "
                    sQuery &= " WHERE ""RPTCODE"" ='AP_AGEING_SUMMARY_CURRENCY'"

                    g_sReportFilename = g_sCustom_APAgeingCurrencyRptFile
            End Select
    ...
    Private Function PrepareDataset() As Boolean
        Try
            ...
            sQuery &= " T1.""CreditLine"", T1.""DebtLine"", T1.""Country"", T1.""Phone1"", "
        ...
        End Try
    End Function
    Private Sub GetCustomizedReportSetting()
        Try
            Dim sQuery As String = ""
            Dim oRec As SAPbobsCOM.Recordset = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)

            g_bCustom_APAgeingCurrency = False
            g_sCustom_APAgeingCurrencyRptFile = ""

            sQuery = "  SELECT  ""RPTCODE"", IFNULL(""FILEPATH"",'') "
            sQuery &= " FROM    """ & oCompany.CompanyDB & """.""@NCM_RPT_CONFIG"" "
            sQuery &= " WHERE   ""RPTCODE"" = 'AP_AGEING_SUMMARY_CURRENCY' AND IFNULL(""INCLUDED"",'N') = 'Y'"
            oRec.DoQuery(sQuery)
            If oRec.RecordCount > 0 Then
                oRec.MoveFirst()
                g_bCustom_APAgeingCurrency = True
                g_sCustom_APAgeingCurrencyRptFile = oRec.Fields.Item(1).Value.ToString.Trim
            End If
            oRec = Nothing
        Catch ex As Exception
            SBO_Application.StatusBar.SetText("[APAgeing].[GetCustomizedReportSetting] :" & ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
        End Try
    End Sub
                ...
                If ExecuteProcedure() Then
                    If PrepareDataset() Then
                        ...
                        Select Case RptType
                            ...
                            Case 2
                                frm.Text = "AP Ageing Summary Report (By Currency)"
                                frm.ReportType = AgeingType.APAgeingSummaryCurrency
                        End Select
    ...
    Private Function ValidateParameter() As Boolean
        Try
            ...
            GetCustomizedReportSetting()
            If g_bCustom_APAgeingCurrency = True Then
                oCombo = oForm.Items.Item("cboRptType").Specific
                Dim RptType As String = oCombo.Selected.Value.ToString.Trim
                If RptType = 2 Then
                    If g_sCustom_APAgeingCurrencyRptFile = "" Then
                        SBO_Application.StatusBar.SetText("Please define the report layout file in IRP Configuration for AP Ageing Summary - By Currency.", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
