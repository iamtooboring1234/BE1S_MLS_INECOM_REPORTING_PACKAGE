...
#Region "Initialization"
    Public Sub ShowForm()
        If LoadFromXML("Inecom_SDK_Reporting_Package.ncmARAgeingBRN.srf") Then
            oForm = SBO_Application.Forms.Item("ncmARAgeingBRN")
...
#Region "General Functions"
        ...
        Try
            ...
            Select Case RptType
                Case 0
                    ...
                    sQuery &= " WHERE ""RPTCODE"" ='" & GetReportCode(ReportName.ARAging_Details) & "'"

                    g_sReportFilename = GetSharedFilePath(ReportName.ARAging_Details)
                ...
                Case 1
                    sQuery = " SELECT IFNULL(""STRUCTUREPATH"",'') FROM ""@NCM_RPT_STRUCTURE"" "
                    sQuery &= " WHERE ""RPTCODE"" ='" & GetReportCode(ReportName.ARAging_Summary_BRN) & "'"

                    g_sReportFilename = GetSharedFilePath(ReportName.ARAging_Summary_BRN)
                    If g_sReportFilename <> "" Then
                        If IsSharedFilePathExists(g_sReportFilename) Then
                            'okay
                        End If
    ...
    Private Function PrepareDataset() As Boolean
        Try
            ...
            Try
                ...
                sQuery &= " IFNULL(T1.""GroupCode"",0) ""GroupCode"", IFNULL(T2.""GroupName"",'') ""GroupName"", IFNULL(T1.""LicTradNum"",'') ""LicTradNum"", "
    ...
    Private Sub LoadViewer()
        ...
        Try
            ...
            Dim sPathFormat As String = "{0}\ARAGEING_{1}.pdf"
            ...
            Try
                ...
                ' set to local
                sTempDirectory = System.Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\ARAGEING\" & oCompany.CompanyDB
                ...
                sFinalFileName = di.FullName & "\ARAGEING_" & sCurrDate & "_" & sCurrTime & ".pdf"
                ' ===============================================================================
                ...
                If (ExecuteProcedure()) Then
                    If PrepareDataset() Then
                        ...
                        frm.ReportName = ReportName.ARAging_Details
                        ...
                        Select Case RptType
                            Case 0
                                frm.Text = "AR Ageing Details Report - Group by BRN"
                                frm.ReportType = AgeingType.ARAgeing
                            Case 1
                                frm.Text = "AR Ageing Summary Report - Group by BRN"
    ...
    Private Function ExecuteProcedure() As Boolean
        ...
        Try
            ...
            sSlsTo = oEdit.Value
            oEdit = oForm.Items.Item("txtBPCode").Specific
            ' sBPCode = IIf(oEdit.Value.Trim = "", "%", "%" & oEdit.Value.Trim.Replace("*", "%") & "%")
        ...
        Try
            ...
            If (bIsExportToExcel) Then
                ...
                While (bIsSaveRunning)
                    System.Threading.Thread.CurrentThread.Sleep(800)
                End While
