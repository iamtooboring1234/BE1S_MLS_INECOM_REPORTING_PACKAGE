delete from [@NCM_QUERY] where U_Type = 'NCM_HYD_OITM'

insert into [@NCM_QUERY] ([code],[name],[u_type],[u_query])
values ('00001111','00001111','NCM_HYD_OITM',
'
select A.ItemCode, A.Itemname, ISNULL(D.AvgPrice,0) As AvgPrice, ISNULL(A.U_ItemCat,'''') AS ItemCat, ISNULL(C.[Name],'''') AS Class,
...
where A.ItemCode = B.ItemCode AND B.BaseType = ''202'' AND C.DocDate >= DATEADD(MONTH, -12, GETDATE())) AS IGEQty,
...
where A.ItemCode = B.ItemCode AND (B.LineStatus = ''O'' OR (B.LineStatus = ''C'' AND B.TargetType =''16'')) AND C.DocDate >= DATEADD(MONTH, -12, GETDATE())) AS DLNQty,
(select ISNULL(SUM(B.Quantity),0) * -1 from [RDN1] B	
LEFT OUTER JOIN [ORDN] C ON B.DOCENTRY = C.DOCENTRY 
where A.ItemCode = B.ItemCode AND C.DocDate >= DATEADD(MONTH, -12, GETDATE())) AS RDNQty,
(select COUNT (DISTINCT B.DOCENTRY) from [IGE1] B		
LEFT OUTER JOIN [OIGE] C ON B.DOCENTRY = C.DOCENTRY 
where A.ItemCode = B.ItemCode AND B.BaseType = ''202'' AND C.DocDate >= DATEADD(MONTH, -12, GETDATE())) As IGESum,
...
LEFT OUTER JOIN [ODLN] C ON B.DOCENTRY = C.DOCENTRY 
where A.ItemCode = B.ItemCode AND (B.LineStatus = ''O'' OR (B.LineStatus = ''C'' AND B.TargetType =''16'')) AND C.DocDate >= DATEADD(MONTH, -12, GETDATE())) As DLNSum,
(select COUNT (DISTINCT B.DOCENTRY) * -1 from [RDN1] B	
LEFT OUTER JOIN [ORDN] C ON B.DOCENTRY = C.DOCENTRY 
where A.ItemCode = B.ItemCode AND C.DocDate >= DATEADD(MONTH, -12, GETDATE())) As RDNSum
From [OITM] A 
LEFT OUTER JOIN [OITW] D ON A.ItemCode = D.ItemCode
LEFT OUTER JOIN [@NCM_ITEMCAT] C ON ISNULL(A.U_ItemCat,'''') = ISNULL(C.Code,'''') 
WHERE A.PrcrmntMtd = ''B''
')