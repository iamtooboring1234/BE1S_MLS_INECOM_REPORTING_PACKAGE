...
#Region "Initialization"
    Public Sub ShowForm()
        If LoadFromXML("Inecom_SDK_Reporting_Package.ncmAPAgeing_Proj.srf") Then
            oForm = SBO_Application.Forms.Item("ncmAPAgeing_Proj")
    ...
    Private Sub SetupChooseFromList()
        ...
        Try
            ...
            oEditLn.ChooseFromListAlias = "PrjCode"
        Catch ex As Exception
            Throw New Exception("[frmApSOA].[SetupChooseFromList]" & vbNewLine & ex.Message)
        End Try
    End Sub
#End Region
...
#Region "General Functions"
    Private Function IsSharedFileExist() As Boolean
        Try
            ...
            Select Case RptType
                Case 0
                    g_sReportFilename = GetSharedFilePath(ReportName.APAging_Details_Proj)
                    ...
                    End If
                Case 1
                    g_sReportFilename = GetSharedFilePath(ReportName.APAging_Summary_Proj)
                    If g_sReportFilename <> "" Then
                        If IsSharedFilePathExists(g_sReportFilename) Then
                            Return True
                        End If
        ...
        Catch ex As Exception
            g_sReportFilename = " "
            SBO_Application.StatusBar.SetText("[A/P AGEING].[GetPath] :" & ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Return False
        End Try
    End Function
    Private Sub GetProjects()
        ...
        Catch ex As Exception
            SBO_Application.StatusBar.SetText("[A/P AGEING].[GetProjects] :" & ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
        End Try
    End Sub
        ...
        Try
            Try
                ...
                If (ExecuteProcedure()) Then
                    ...
                    oUpdate.DoQuery("UPDATE " & oCompany.CompanyDB & ".""@NCM_AP_AGEING"" SET ""PROJECT"" = '<Blank>' WHERE IFNULL(""PROJECT"", '') = '' AND ""USERNAME"" ='" & g_sAPAGERunningDate & oCompany.UserName.Trim & "' ")

                    oUpdate = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                    oUpdate.DoQuery("UPDATE " & oCompany.CompanyDB & ".""@NCM_AP_AGEING"" SET ""NUMATCARD"" = '' WHERE IFNULL(""NUMATCARD"", '') = '' AND ""USERNAME"" ='" & g_sAPAGERunningDate & oCompany.UserName.Trim & "' ")

                    oUpdate = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                    oUpdate.DoQuery("UPDATE " & oCompany.CompanyDB & ".""@NCM_AP_AGEING"" SET ""REF2"" = '' WHERE IFNULL(""REF2"", '') = '' AND ""USERNAME"" ='" & g_sAPAGERunningDate & oCompany.UserName.Trim & "' ")
                    ...
                    If RptType = 0 Then
                        frm.Text = "AP Ageing Details Report With Project"
                        frm.ReportType = AgeingType.APAgeing
                    ElseIf RptType = 1 Then
                        frm.Text = "AP Ageing Summary Report With Project"
                        frm.ReportType = AgeingType.APAgeingSummary
                    End If

                    frm.ReportName = ReportName.APAging_Details_Proj
    ...
    Private Function ExecuteProcedure() As Boolean
        ...
        Try
            ...
            Return True
        Catch ex As Exception
            SBO_Application.MessageBox("[frmAPAgeing][ExecuteProcedure]:" & ex.ToString)
            Return False
        End Try
    End Function
        ...
        Try
            ...
            For iCount = 1 To 4
                ...
                If (i1 < -1) Then
                    ...
                    SBO_Application.StatusBar.SetText("[APAgeing][ValidateParameters] - Value in bucket " & iCount.ToString() & " is less than zero", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    Return False
                End If

                If (i2 < -1) Then
                    oForm.ActiveItem = sTemp2
                    SBO_Application.StatusBar.SetText("[APAgeing][ValidateParameters] - Value in bucket " & (iCount + 1).ToString() & " is less than zero", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    Return False
                End If

                If (i1 > i2) Then
                    oForm.ActiveItem = sTemp1
                    SBO_Application.StatusBar.SetText("[APAgeing][ValidateParameters] - Value in bucket " & (iCount).ToString() & " is greater than value in bucket " & (iCount + 1).ToString(), SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    Return False
                End If
                ...
                If (iCount = 4) Then
                    If (i1 <> i2) Then
                        oForm.ActiveItem = sTemp1
                        SBO_Application.StatusBar.SetText("[APAgeing][ValidateParameters] - Value in bucket " & (iCount).ToString() & " is not equal to value in bucket " & (iCount + 1).ToString(), SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                        Return False
                    End If
                End If
            Next
